...
export function InterviewsList({ candidateId }: InterviewsListProps) {
  ...
  const vacancies = useSuspenseQueries({
    queries: interviews.data.filter(interview => interview.vacancy_id).map(interview => vacancyQueryOptions(interview.vacancy_id!)),
  });
        ...
        {interviews.data.map((interview, index) => (
          <Card key={interview.id}>
            <CardHeader className="flex flex-row items-center">
              <CardTitle>
                <Link to="/vacancies/$vacancyId" params={{ vacancyId: interview.vacancy_id!.toString() }}>
            ...
            <CardContent>
              {interview.created_at && <div>
                Создано <RelativeTimeTooltip date={new Date(interview.created_at)} />
              </div>}
            </CardContent>
          </Card>
        ))}
