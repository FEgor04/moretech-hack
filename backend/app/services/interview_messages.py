...
class InterviewMessagesService:
    ...
    async def create_message(
        ...
        try:
            ...
            ai_response = await self._get_ai_response(session, interview_id, payload.text)
    ...
    def _create_system_prompt(self, candidate: Candidate, vacancy: Vacancy | None) -> str:
    ...
    async def _get_initial_greeting(self, system_prompt: str) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to get initial greeting from GigaChat: {e}", exc_info=True)
            raise
