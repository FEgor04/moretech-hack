...
def upgrade() -> None:
    ...
    if inspector.has_table("candidate"):
        ...
        # Add new columns
        op.add_column("candidate", sa.Column("position", sa.String(length=255), nullable=False, server_default=""))
        op.add_column("candidate", sa.Column("experience", sa.Integer(), nullable=False, server_default="0"))
...
def downgrade() -> None:
    ...
    if inspector.has_table("candidate"):
        ...
        op.add_column("candidate", sa.Column("resume_url", sa.String(length=1024), nullable=True))
        op.add_column("candidate", sa.Column("notes", sa.Text(), nullable=True))
        op.add_column("candidate", sa.Column("user_id", sa.Integer(), sa.ForeignKey("user.id"), nullable=True))
