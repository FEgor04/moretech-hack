...
class TestPDFParserService:
    ...
    def test_parse_vacancy_success(self):
        ...
        mock_chat_response = Mock()
        mock_choice = Mock()
        mock_choice.message.content = '{"title": "Senior Developer", "description": "We need a senior developer"}'
        mock_chat_response.choices = [mock_choice]
        mock_client.chat.return_value = mock_chat_response
    ...
    def test_analyze_cv_with_gigachat(self):
        ...
        mock_chat_response = Mock()
        mock_choice = Mock()
        mock_choice.message.content = "This candidate has 5 years of experience in Python and FastAPI."
        mock_chat_response.choices = [mock_choice]
        mock_client.chat.return_value = mock_chat_response
        ...
        # Test analysis
        result = parser.analyze_cv_with_gigachat("test-file-id-123", "What are this candidate's main skills?")
        
        # Verify result
        assert result == "This candidate has 5 years of experience in Python and FastAPI."
        
        # Verify GigaChat client was called correctly
        mock_client.chat.assert_called_once()
        call_args = mock_client.chat.call_args[0][0]
        assert call_args["messages"][0]["attachments"] == ["test-file-id-123"]
        assert call_args["messages"][0]["content"] == "What are this candidate's main skills?"
    ...
    def test_analyze_vacancy_with_gigachat(self):
        ...
        # Test analysis
        result = parser.analyze_vacancy_with_gigachat("test-vacancy-id-456", "What are the main requirements for this position?")
        
        # Verify result
        assert result == "This position requires 3+ years of Python experience and knowledge of FastAPI."
        
        # Verify GigaChat client was called correctly
        mock_client.chat.assert_called_once()
        call_args = mock_client.chat.call_args[0][0]
        assert call_args["messages"][0]["attachments"] == ["test-vacancy-id-456"]
        assert call_args["messages"][0]["content"] == "What are the main requirements for this position?"
